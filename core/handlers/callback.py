...
async def select_category_for_order(
    call: CallbackQuery, callback_data: City, state: FSMContext
):
...
async def select_category_for_calculation(
    call: CallbackQuery, callback_data: Category, state: FSMContext
):
...
async def select_return_to_main_menu(call: CallbackQuery):
    await call.message.answer(
        f"{hbold(call.from_user.first_name)}, возвращаемся в главное меню!",
